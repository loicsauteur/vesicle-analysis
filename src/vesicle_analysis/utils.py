...
def get_nucleus_volume(img_nucleus: np.ndarray, cal: tuple):
    ...
    Parameters
    -------
    img_nucleus = label image of nucleus
    cal = tuple of ZYX pixle size in µm

    Returns
    -------
    Tuple: number of voxels, volume in calibrated units

    """
    props = regionprops(img_nucleus)
    if len(props) > 1:
        raise RuntimeError(f'Could not identify nucleus size, as there are {len(props)} objects')
    elif len(props) == 0:
        raise RuntimeError("No objects in the image")
    return props[0].area, props[0].area * cal[0] * cal[1] * cal[2]


def min_max_distance(img_label, calibration: tuple):
    """
    Finds the min and max distance between multiple object centrois (in calibrated units).
    
    Parameters
    -------
    img_label = label image of vesicles
    calibration = tuple of ZYX pixle size in µm

    Returns
    -------
    dictionary with min/max distance per label
    """
    ves_ves_dist = {'label': [], 'min_ves-ves-dist': [], 'max_ves-ves-dist': []}
    for cur_region in regionprops(img_label):
        ...
        for region in regionprops(img_label):
            if not cur_region.label == region.label:
                ...
                cur_dist = (abs(x1 - x2) ** 2 + abs(y1 - y2) ** 2 + abs(z1 - z2) ** 2) ** (1/2)
...
def distance_p2p(p1: tuple, p2: tuple, calibration: tuple):
    """
    Given two points with each 3D coordinates, calculates the distance in calibrated units

    Parameters
    -------
    p1 = tuple (Z, Y, X) coordinates
    p2 = tuple (Z, Y, X) coordinates
    calibration = tuple of ZYX pixle size in µm

    Returns
    -------
    float: distance between p1 and p2 in µm
...
def get_angle_and_distance(df_nuc: pd.DataFrame, df_ves: pd.DataFrame, calibration: tuple):
    """
    With the centroid(s) of the nuceus and vesicles, the function will calculate the:
    - distance betweeen nucleus and vesicle centroids in 3D (in µm)
    - calculate the angle (2D, i.e. projection) between nucleus and vesicle centroid
    - normalise the angle using the mean angle or the middle angle (middle between min and max anlges)
    - and absolute normalised angles (changing negative angles to positive)

    Parameters
    -------
    df_nuc: pd.DataFrame containing the Nucleus centroids (1-3)
    df_ves: pd.DataFrame containing the vesicle centroids (1-3)
    calibration: tuple of ZYX pixel sizes in µm
    
    Returns
    -------
    dict: with values for vesicles, 
          keys = [label, distance-NucCentroid, angle, angleNorm2Mean, angleNorm2Middle, abs(angleNorm2Mean), abs(angleNorm2Middle)]
    ...
    if len(df_nuc) != 1:
        print('>> WARNING! there is not exactly one nucleus in the image --- skipping angle measurement <<')
        return None
    ...
    # calculate per vesicle angle the deviation from (1) the average angle, (2) the middle angle
    for i in range(len(df_ves)):
...
######## ---------------
def do_measurements(
    nucleus_mask, nucleus_ch,
    vesicle_mask, vesicle_ch,
    calibration: tuple
    ):
    """
    ...
    Parameters
    -------
    nucleus_mask = label image
    nucleus_ch = intensity image
    vesicle_mask = label image
    vesicle_ch = intensity image
    calibration = tuple of ZYX pixel size in µm

    Returns
    -------
    pd.Dataframe: combined table for nucleus & vesicle measuremetns
    pd.Dataframe: table only of the nucleus measurements
    pd.Dataframe: table only of the vesicle measurements
    
    """
    # props to measure
    props = ('label', 'area', 'area_filled', 'area_convex', 'centroid', 'intensity_mean', 'intensity_max', 'intensity_min',
             
             #'axis_major_length', 'axis_minor_length',
             # not for 3D: 'eccentricity', 'orientation', 'perimeter', 'perimeter_crofton'
            )
    voxelsize = calibration[0] * calibration[1] * calibration[1]
    calibration_dict = {
        'label': 1, 'area': voxelsize, 'area_filled': voxelsize, 'area_convex': voxelsize,
        'centroid-0': 1, 'centroid-1': 1, 'centroid-2': 1, 
        'intensity_mean': 1, 'intensity_max': 1, 'intensity_min': 1,

        #'axis_major_length':1, 'axis_minor_length':1, 
        # not for 3D: 'eccentricity':1, 'orientation':1, 'perimeter':1, 'perimeter_crofton':1
    }

    # Nucleus measurements       ----------------------------------
    nuc_table = regionprops_table(nucleus_mask, intensity_image=nucleus_ch, properties=props)
    nuc_table = pd.DataFrame(nuc_table)

    # convert volumes to metric units (not pixels -> basically only 'area' & 'area_filled' colums)
    nuc_table = nuc_table.mul(calibration_dict)

    # Calculate nucleus roundness in 2D, and ellipse axes
    nuc_2d_props = regionprops(np.max(nuc_mask, axis=0)) # props of projection
    nuc_circ_dict = {'label': [], 'Total n vesicles': [], 'Image stack height': [],
                     '2D_circularity': [], '2D_circularity_crofton': [],
                     '2D_ellipse_major': [], '2D_ellipse_minor': [], 'ratio_2D_minor-major': [],
                     '2D area': []
                    }
    for p in nuc_2d_props:
        ...
        nuc_circ_dict['ratio_2D_minor-major'].append(p.axis_minor_length / p.axis_major_length)
        # add also the projected object area
        nuc_circ_dict['2D area'].append(p.area * calibration[1] * calibration[2])
        
    
    # Vesicle measurements        ----------------------------------
    ves_table = regionprops_table(vesicle_mask, intensity_image=vesicle_ch, properties=props)
    ...
    if len(ves_table['label']) == 0:
        print(f'>>>>WARNING: No vesicles identified.<<<<<<')
        ...
        for label in ves_table['label']:
            ...
            ves_2D['2D area'].append(single_props[0].area * calibration[1] * calibration[2])
        # merge the 2D area measurements with the ves_table
        ves_table = pd.merge(ves_table, pd.DataFrame.from_dict(ves_2D), on='label')
    else:
        print(f'>>>>WARNING: did not perform 2D vesicle measurement because there more than 100 vesicles in the image.<<<<<<')
    ...
    ves_ves_dist = p2p_distances(vesicle_mask, calibration=calibration)
    ...
    for region in regionprops(vesicle_mask):
        ...
        dict_dist['distance-NucSurface'].append(distance[int(region.centroid[0])][int(region.centroid[1])][int(region.centroid[2])] * calibration[1])
    ...
    # Add the total vesicle count to the nucleus table (via the nucleus circularity diction)
    nuc_circ_dict['Total n vesicles'] = [len(ves_table['label'])] * len(nuc_circ_dict['label'])

    # add stack height in um to the nucleus table
    nuc_circ_dict['Image stack height'] = [(nucleus_ch.shape[0] - 1) * calibration[0]] * len(nuc_circ_dict['label'])
    ...
    df_image = pd.concat([nuc_table, ves_table], keys=['Nuclei', 'Vesicles'], names=['Object', 'Object ID'])
    # Rename the table headers for 'area' to volume
    df_image = df_image.rename(columns={'area': 'volume', 'area_filled': 'volume (filled)', 'area_convex': 'volume (convex hull)'})
...
def merge_csv_files(dir_path: str, save_string: str = ''):
    """
    Looks for CSV files with the proper version tag in the name and merges them into a single talbe.

    Patrameters
    ----------
    dir_path: String of path to folder
    save_string: (Optional) String for additional info in saving file name

    """
    all_dfs = []
    for file in os.listdir(dir_path):
        if file.endswith(__version__ + '.csv') and not file.startswith('Merged_tables_'):
    ...
    # Save merged table
    merged_name = 'Merged_tables_' + save_string + '_v' + __version__ + '.csv'
